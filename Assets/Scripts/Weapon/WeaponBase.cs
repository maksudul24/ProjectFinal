	...
	void Fire() {
		...
		for(int i = 0; i < pellets; i++) {
			if(Physics.Raycast(shootPoint.position, CalculateSpread(spread, shootPoint), out hit, range)) {
				if(CommonScript.isMultiplayer == false && CommonScript.isCO_Op == false){
				...
				//MultiPlayer
					if(hit.transform.tag == "Player" || hit.transform.tag == "Enemy"){
						Debug.Log("RPC FUntioncalled");
						damage_count = GetWeaponDamage();
						hit.transform.GetComponent<PhotonView>().RPC("ApplyDamage",RpcTarget.AllBuffered,damage_count);
					}
					else {
						CreateRicochet(hit.point, hit.normal);
